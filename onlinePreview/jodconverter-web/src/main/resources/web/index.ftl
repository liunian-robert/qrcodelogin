...
<script>
    ...
    function getFastDfsUrl(fileName) {

    }
    function previewFile(fileName) {
        $.ajax({
            url: '${baseUrl}fastdfsUrl?fileName=' + encodeURIComponent(fileName),
            success: function (data) {
                // 删除完成，刷新table
                if (1 == data.code) {
                    alert(data.msg);
                    ...
                    var url = "${baseUrl}onlinePreview?url=" + encodeURIComponent(body.content);
                    var winHeight = window.document.documentElement.clientHeight-10;
                    window.open(url, "_blank", "height=" + winHeight
                            + ",top=80,left=80,toolbar=no, menubar=no, scrollbars=yes, resizable=yes");
                }
    ...
    $(function () {
        $('#table').bootstrapTable({
            ...
            $(data).each(function (index, item) {
                item.action = "<a class='btn btn-default' target='_blank' href='javascript:void(0);' onclick='previewFile(\""+item.fileName+"\")'>预览</a>" +
